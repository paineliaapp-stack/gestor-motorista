            ...
            return (
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 12px', background:'rgba(255,255,255,0.02)' }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <ViewsBar value={v.views||0} max={maxViews} color={ytGlow} />
                </div>
...
export function NicheWorld() {
        ...
        <div style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(0,0,0,0.75)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div style={{ width:'100%', maxWidth:500, background:'#0a1a16', border:'1px solid rgba(0,229,176,0.2)', borderRadius:16, padding:24, display:'flex', flexDirection:'column', gap:16, maxHeight:'90vh', overflowY:'auto' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              ...
              <div>
                <p style={{ fontFamily:'-apple-system,sans-serif', fontSize:11, color:'rgba(255,255,255,0.4)', margin:'0 0 6px', letterSpacing:'0.08em' }}>CHANNEL ID</p>
                <input value={channelId} onChange={e=>setChannelId(e.target.value)} placeholder="UCxxxxxxxxxxxxxxxxxxxxxxxx" style={{ width:'100%', padding:'10px 14px', borderRadius:8, background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'#fff', fontFamily:"Space Mono,monospace", fontSize:12, outline:'none', boxSizing:'border-box' }} />
              </div>
            </div>
  ...
  localStorage.setItem('vn_yt_channel_id_'+activeNiche.id, channelId.trim().replace(/\/+$/, ''));
  if(apiKey && channelId){
    fetch('/api/youtube?apiKey='+encodeURIComponent(apiKey)+'&channelId='+encodeURIComponent(channelId))
      .then(r=>r.json()).then(d=>{ setYtData(d); try { localStorage.setItem('vn_yt_data_'+activeNiche.id, JSON.stringify(d)); } catch {} }).catch(()=>{});
  }
}
setShowApiModal(false); }} style={{ flex:2, padding:'10px', borderRadius:8, background:'rgba(0,229,176,0.15)', border:'1px solid rgba(0,229,176,0.3)', color:'#00e5b0', fontFamily:'-apple-system,sans-serif', fontSize:13, fontWeight:600, cursor:'pointer' }}>💾 Salvar</button>
            </div>
          </div>
        </div>
      )}
